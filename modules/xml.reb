...
load-xml: use [
	...
	xml!: context [
		this: name: space: value: tree: branch: position: blank
		...
		find-element: func [element [tag! issue! datatype! word!] /local hit][
			parse value [
				any [and path! hit: into [element] break | (hit: blank) skip]
			]
			hit
		]
		...
		parent: func [/local branch]["Need Branch" blank]

		children: func [/local hits hit][
			hits: copy []
			parse case [
				block? value [value] string? value [reduce [%.txt value]] blank? value [[]]
			][
				any [is-attr skip]
		...
		path: func [path [block! path!] /local result selector kids][
			...
			unless parse path [
				opt [tag! (either result/name = path/1 [result: compose [(any [:result []])]][result: blank])]
		...
		attributes: func [/local hits hit][
			hits: copy []
		...
		append-child: func [name data /attr /local pos][
			case [
				blank? position/2 [value: tree/2: position/2: copy []]
				string? position/2 [
		...
		append-text: func [text][
			case [
				blank? position/2 [value: tree/2: position/2: text]
	...
	doc: make xml! [
		...
		document: true
		new: does [clear branch tree: position: reduce ['document blank]]

		open-tag: func [tag][
			insert/only branch position
			tag: name-to-tag tag
			tag/1: to-tag tag/1
			tree: position: append-child tag blank
		]
	...
	load-xml: func [
		...
		root: doc/new
		parse/case document [header element to end]
